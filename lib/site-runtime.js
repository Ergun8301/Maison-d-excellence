...
export class SiteRuntime {
  constructor(page) {
    this.page = page;
    this._slide = 0;
    this.props = { heroAlign: 'Centré', heroVoile: 46 };
  }
  ...
  scrollToId(id, tries) {
    ...
    const off = (hdr ? hdr.getBoundingClientRect().height : 76) + 14;
    this.smoothTo(el.getBoundingClientRect().top + window.pageYOffset - off);
  }
