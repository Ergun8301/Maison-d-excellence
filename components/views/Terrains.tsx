...
export default function Terrains(v: Vals) {
  const { goContact, hasTerrains, sansTerrains, terrainChips, terrainCount, terrains } = v;
  return (
    <>
      <main data-screen-label="Terrains">
          <section style={{ position: "relative", minHeight: "clamp(480px,66vh,600px)", overflow: "hidden", display: "flex", alignItems: "flex-end" }}>
            ...
            <div style={{ position: "relative", width: "100%", maxWidth: "1440px", margin: "0 auto", padding: "clamp(104px,11vw,128px) clamp(20px,5vw,64px) clamp(44px,5.4vw,76px)" }}>
              ...
              <p data-reveal="" data-delay="220" style={{ margin: "clamp(20px,2.2vw,28px) 0 0", color: "rgba(247,247,244,0.86)", fontSize: "clamp(15px,1.35vw,17.5px)", lineHeight: "1.7", maxWidth: "60ch", textShadow: "0 2px 20px rgba(11,13,12,0.85)" }}>Nous ne vendons pas de terrains : nous construisons dessus. Nous suivons les parcelles qui se libèrent dans le secteur et vérifions leur faisabilité pour votre projet de maison.</p>
            </div>
          </section>
          ...
          <section style={{ background: "#F7F7F4", padding: "clamp(64px,8vw,110px) 0 clamp(72px,9vw,130px)" }}>
            <div style={{ maxWidth: "1440px", margin: "0 auto", padding: "0 clamp(20px,5vw,64px)" }}>
              {(hasTerrains) ? (<><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "24px", marginBottom: "clamp(28px,3.4vw,40px)" }}>
                <div>
                ...
                {(terrains ?? []).map((t, i) => (
                  <a href={t.href} onClick={t.onOpen} data-reveal="" data-delay={t.delay} className="me-card" style={{ display: "block", textDecoration: "none", borderRadius: "9px", overflow: "hidden", background: "#FFFFFF", border: "1px solid rgba(17,20,18,0.07)", opacity: `${t.cardOpacity}`, color: "#111412" }} key={i}>
                    <div style={{ position: "relative", overflow: "hidden", aspectRatio: "16/11" }}>
                      <img loading="lazy" className="me-zoom" src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg'%3E%3C/svg%3E" alt={`Terrain à bâtir à ${t.commune} dans l'Ain`} style={{ width: "100%", height: "100%", objectFit: "cover", backgroundColor: "#EDEDE7", backgroundSize: "cover", backgroundPosition: "center", backgroundImage: `${t.bg}` }} />
              ...
              <p data-reveal="" style={{ margin: "clamp(34px,4vw,48px) 0 0", fontSize: "13px", color: "#8D948E", lineHeight: "1.6", maxWidth: "78ch" }}>Informations communiquées à titre indicatif, hors frais de notaire et sous réserve de disponibilité. Les terrains sont commercialisés par leurs propriétaires ou par notre partenaire foncier : vous restez libre de votre constructeur.</p></>) : null}
